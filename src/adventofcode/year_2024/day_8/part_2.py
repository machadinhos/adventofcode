...
def main():
    ...
    unique_locations = {coords for coords_list in towers.values() for coords in coords_list if len(coords_list) > 1}
    for (x1, y1), (x2, y2) in towers_coords_loop(towers):
        vx, vy = x2 - x1, y2 - y1
        new_x, new_y = x1, y1
        while 0 <= (new_x := new_x - vx) < width and 0 <= (new_y := new_y - vy) < height:
